...
Last Quarter Avg: ${lastQuarterAvg.toFixed(2)}`;
  ...
  return (
    <div style={{ maxWidth: "850px", margin: "auto", padding: "25px", fontFamily: "Segoe UI" }}>
      ...
      <button onClick={analyzeAndForecast}>Analyze & Forecast (Auto Save)</button>
      ...
      {forecast.length > 0 && (
        <div style={{ marginTop: "20px" }}>
          ...
          <table border="1" cellPadding="5" style={{ width: "100%", marginBottom: "10px" }}>
            ...
            <tbody>
              {forecast.map((f, i) => (
                <tr key={i}><td>{f.month}</td><td>{f.value}</td></tr>
              ))}
            </tbody>
          </table>
            ...
            <tbody>
              {savedForecasts.map((f) => (
                <tr key={f._id}>
                  ...
                  <td>
                    <button onClick={() => deleteForecast(f._id)} style={{ color: "red" }}>
                      🗑️ Delete
                    </button>
                  </td>
                </tr>
