...
/*
  HelpdeskSystem (in-memory storage + live SLA)
  - Keeps tickets & articles in state
  - SLA countdown updated every 30s; auto-escalates overdue tickets
  - Ticket detail supports internal notes, messages, status changes, reassign, SLA update
  - Customer Communication History with full conversation tracking
...
export default function HelpdeskSystem() {
  ...
  const RESPONSE_TEMPLATES = [
    { id: 1, name: "Received", text: "Thank you for contacting us. We have received your request and will respond shortly." },
    { id: 2, name: "In Progress", text: "We are currently working on your request. We'll update you as soon as we have more information." },
    { id: 3, name: "Follow-up", text: "We wanted to follow up on your previous request. Is there anything else we can help you with?" },
    { id: 4, name: "Resolved", text: "Your issue has been resolved. Please let us know if you need any further assistance." },
    { id: 5, name: "Escalated", text: "Your request has been escalated to our senior support team for priority handling." }
  ];
  ...
  // send message (customer communication history)
  const sendMessage = (id, text, from = "Agent", isAutomated = false) => {
    if (!text || !text.trim()) return;
    const message = { 
      id: uid(), 
      from, 
      ...
      agentName: from === "Agent" ? (detailTicket?.agent || "Unassigned") : from
    };
    setTickets((prev) => prev.map((t) => (t.id === id ? { ...t, messages: [...(t.messages || []), message] } : t)));
    setDetailTicket((d) => ({ ...d, messages: [...(d.messages || []), message] }));
    setMessageText("");
  ...
  const sendTemplate = (id, templateText) => {
    sendMessage(id, templateText, "Agent", true);
    setShowTemplates(false);
  };
  
  // get customer full history (all tickets and messages)
  const getCustomerHistory = (customerName) => {
    return tickets
      .filter(t => t.customer.toLowerCase() === customerName.toLowerCase())
      .sort((a, b) => new Date(b.created) - new Date(a.created));
  };
  
  // view customer history
  const viewCustomerHistory = (customerName) => {
      ...
      <main className="max-w-5xl mx-auto p-6">
            ...
            {showDetail && detailTicket && (
              <div className="w-full md:w-[35%] bg-white rounded-xl border-2 border-blue-300 shadow p-5 sticky top-6 self-start">
                ...
                <div className="text-sm mb-1">
                  <b>Customer:</b> {detailTicket.customer}
                  <button 
                    onClick={() => viewCustomerHistory(detailTicket.customer)}
                ...
                <div className="flex gap-2 mb-1">
                  ...
                  >
                    {showTemplates ? "Hide Templates" : "Quick Responses"}
                  </button>
                </div>
                
                {showTemplates && (
                  <div className="bg-blue-50 p-2 rounded mb-2 max-h-32 overflow-y-auto">
                    <div className="text-xs font-semibold mb-1 text-gray-700">Select a template:</div>
                    {RESPONSE_TEMPLATES.map(template => (
                      <button
                        key={template.id}
                        onClick={() => sendTemplate(detailTicket.id, template.text)}
                        className="block w-full text-left text-xs bg-white hover:bg-blue-100 px-2 py-1 mb-1 rounded border"
                      >
                        <span className="font-semibold">{template.name}:</span> {template.text.substring(0, 50)}...
                      </button>
                    ))}
                  </div>
                )}
                ...
                <ul className="max-h-28 overflow-y-auto text-sm border rounded p-2 bg-gray-50">
                  {(detailTicket.messages || []).map((m) => (
                    <li key={m.id} className="mb-2 pb-1 border-b border-gray-200 last:border-0">
                      <div className="flex items-center gap-2">
                        <b className="text-blue-700">{m.from}:</b>
                        {m.automated && <span className="text-xs bg-green-100 text-green-700 px-1 rounded">Auto</span>}
                        {m.agentName && m.from === "Agent" && (
                          <span className="text-xs text-gray-500">({m.agentName})</span>
                        )}
                      </div>
                  ...
                  {(!detailTicket.messages || detailTicket.messages.length === 0) && (
                    <li className="text-gray-400 text-xs text-center py-2">No messages yet</li>
                  )}
                </ul>
                <div className="flex gap-2 mt-2">
                  <input
                  ...
                  <button
                    onClick={() => sendMessage(detailTicket.id, messageText)}
                    className="bg-blue-600 text-white rounded px-3 py-2 text-sm hover:bg-blue-800 transition"
                  >
                    Send
                  </button>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mt-4 justify-center">
                  ...
                  <button
                    onClick={() => removeTicket(detailTicket.id)}
                    className="bg-red-600 text-white rounded px-3 py-1 hover:bg-red-700 transition"
                  >
                    Remove
                  </button>
                </div>
        ...
        {panel === "customers" && (
          <section className="bg-white rounded-xl shadow-lg p-6">
            <h2 className="text-2xl font-bold mb-4 text-blue-700 text-center border-b pb-3">
              👥 Customer Communication History
            </h2>
            
            {/* Customer List */}
            <div className="mb-6">
              <h3 className="font-semibold text-lg mb-3">Select a Customer:</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {[...new Set(tickets.map(t => t.customer))].filter(c => c).map(customer => {
                  const customerTickets = getCustomerHistory(customer);
                  const totalMessages = customerTickets.reduce((sum, t) => sum + (t.messages?.length || 0), 0);
                  const lastContact = customerTickets[0]?.created;
                  
                  return (
                    <div 
                      key={customer}
                      onClick={() => viewCustomerHistory(customer)}
                      className="border-2 border-blue-200 rounded-lg p-4 cursor-pointer hover:bg-blue-50 transition"
                    >
                      <div className="font-bold text-lg text-blue-700">{customer}</div>
                      <div className="text-sm text-gray-600 mt-1">
                        <div>Tickets: {customerTickets.length}</div>
                        <div>Messages: {totalMessages}</div>
                        <div className="text-xs text-gray-400 mt-1">
                          Last contact: {lastContact ? new Date(lastContact).toLocaleDateString() : "N/A"}
                        </div>
                      </div>
                    </div>
                  );
                ...
                {tickets.length === 0 && (
                  <div className="col-span-3 text-center text-gray-400 py-8">
                    No customers yet. Create a ticket to get started.
                  </div>
                )}
              </div>
            </div>
            ...
            {showCustomerHistory && selectedCustomer && (
              <div className="border-2 border-blue-300 rounded-lg p-6 bg-blue-50">
                ...
                <div className="space-y-4 max-h-96 overflow-y-auto">
                  {getCustomerHistory(selectedCustomer).map(ticket => (
                    <div key={ticket.id} className="bg-white rounded-lg p-4 shadow">
                      ...
                      <div className="mt-3 border-t pt-2">
                        ...
                        <div className="text-sm text-gray-700 mb-3">{ticket.description}</div>
                        
                        {ticket.messages && ticket.messages.length > 0 && (
                          <>
                            <div className="font-semibold text-sm mb-1">
                              Communication ({ticket.messages.length} messages):
                            </div>
                            <div className="bg-gray-50 rounded p-2 max-h-40 overflow-y-auto">
                              {ticket.messages.map(msg => (
                                <div key={msg.id} className="text-xs mb-2 pb-1 border-b last:border-0">
                                  <div className="flex items-center gap-2">
                                    <span className="font-semibold text-blue-600">{msg.from}</span>
                                    {msg.automated && <span className="bg-green-100 text-green-700 px-1 rounded">Auto</span>}
                                    {msg.agentName && <span className="text-gray-500">({msg.agentName})</span>}
                                    <span className="text-gray-400 ml-auto">
                                      {new Date(msg.time).toLocaleString()}
                                    </span>
                                  </div>
                                  <div className="text-gray-700 mt-1">{msg.text}</div>
                                </div>
                              ))}
                            </div>
                          </>
                        ...
                        {ticket.updates && ticket.updates.length > 0 && (
                          <>
                            <div className="font-semibold text-sm mt-2 mb-1">Updates:</div>
                            <div className="text-xs text-gray-600">
                              {ticket.updates.slice(-3).map((update, i) => (
                        ...
                        <div className="mt-2 text-xs text-gray-500">
                          Assigned to: <span className="font-semibold">{ticket.agent || "Unassigned"}</span>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <section className="bg-white rounded shadow p-4">
            ...
            <table className="w-full border">
              ...
              <tbody>
                ...
                {Object.keys(slaReport).length === 0 && (
                  <tr>
                    <td colSpan={2} className="border px-2 py-4 text-center text-gray-400">
                      No tickets yet
                    </td>
                  </tr>
                )}
